...
def create_invoice_pdf(INVOICE, DETAILS, FILEPATH):
    ...
    pdf.text(x=50, y=22, txt='1/16, Vijay Nagar, Bandrekarwadi, Jogeshwari (East), Mumbai 400060.')
    ...
    for row in DETAILS:  # sets no. of rows
        ...
        for field in deets_w:  # sets no. of cols
            pdf.cell(deets_w[field] * 2, 6, str(row[field]), border=1, align='C', fill=False)
        pdf.ln(2)
    ...
    pdf.cell(90, 6, "Goods once sold cannot be returned or replaced", border="T", align='C', fill=False)
    pdf.cell(20, 6, '', border=0, align='L', fill=False)
    
    pdf.set_font('Arial', '', 9.0)

    pdf.cell(40, 6, 'Total Tax Amount:', border=1, align='L', fill=False)
    pdf.cell(40, 6, str(INVOICE["total_tax_amt"]), border=1, align='C', fill=False)
    ...
    pdf.cell(40, 6, str(INVOICE["total_after_tax"]), border=1, align='C', fill=False)
    ...
    pdf.cell(40, 6, str(INVOICE["gst_reverse_charge"]), border=1, align='C', fill=False)
