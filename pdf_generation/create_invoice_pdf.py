...
def create_invoice_pdf(INVOICE, DETAILS, FILEPATH):
    ...
    pdf.text(x=50, y=22, txt='1/16, Vijay Nagar, Bandrekarwadi Jogeshwari, (East), Mumbai - 400 060.')
    ...
    pdf.text(x=55, y=260, txt=str(INVOICE["ifsc"]))

    pdf.line(10, 265, 100, 265)

    pdf.set_font('Times', 'IB', 9.0)
    notice = 'Goods once sold cannot be returned or replaced'
    ...
    for field in right_bottom_headers:
        ...
        pdf.set_font('Times', 'IB', 9.0)
        pdf.cell(118)
        pdf.cell(18 * 2, 6, field.replace("_", " ").title(), border=1, align='R', fill=False)
        pdf.set_font('Arial', '', 9.0)
        pdf.cell(18 * 2, 6, str(INVOICE[field]), border=1, align='C', fill=False)
        pdf.ln(2)
